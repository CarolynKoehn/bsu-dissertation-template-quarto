...
Please consult the articles by Admirer (Admirer, 1970) and Admirer,
Smith and Doe (Admirer, Smith, \& Doe, 1972) for more details. Note that
the references are cited by the last names of all authors for three
...
processing with packages \texttt{tidyverse} and \texttt{kableExtra}
(Table~\ref{tbl-CSPV}).
...
Here's an example of a table using raw \LaTeX~code with no Quarto
processing (Table~\ref{tbl-SPWC}).
